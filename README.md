...
- Maven 3.8+
- OpenAI API Key
- Python 3.8+ (опционально, для точного анализа формант)
...
### Анализ дикции и формант
- **Встроенный FFT анализ** (базовая точность, работает всегда)
- **Python-сервис с Praat** (высокая точность ±5-10 Hz, опционально)

#### Запуск Python-сервиса для точного анализа формант:
...
Сервис будет доступен на `http://localhost:8041`. Kotlin бэкенд автоматически использует его, если он доступен, иначе переключается на встроенный FFT анализ.

**Преимущества Praat:**
- Точность ±5-10 Hz (вместо ±50-100 Hz)
- LPC анализ (стандарт в фонетике)
- Автоматическая фильтрация гармоник
- Дополнительные метрики (F0, bandwidth, F3, F4)
